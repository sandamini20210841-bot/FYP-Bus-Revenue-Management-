...
-- 1) Ensure ticket_number column exists.
ALTER TABLE tickets
ADD COLUMN IF NOT EXISTS ticket_number VARCHAR(9);

-- 2) Backfill missing ticket numbers with deterministic unique values.
WITH numbered AS (
...
-- 3) Guarantee uniqueness / non-null before moving primary key.
DO $$
BEGIN
...
-- 4) Add new FK-compatible column in transactions and copy values.
...
-- 5) Drop old FK/column and rename new one.
ALTER TABLE transactions
...
-- 6) Switch primary key to ticket_number and remove old id column.
ALTER TABLE tickets
